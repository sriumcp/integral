...
function systemPreamble(): string {
  return [
    ...
    '     story given the available datasets.',
    '  2. Request scalars (named numeric values + a few const-strings) that',
    '     the prose will reference.',
    '  3. Author a `prose_template` that uses ONLY the scalars + excerpts',
    '     you requested. The substrate will substitute them in.',
    '',
    'CRITICAL DISCIPLINE — you are an analyst, never a calculator:',
    '  - You MUST NOT write any digit or numeric value directly into the prose.',
    '    Use {scalar:id} placeholders for every number.',
    ...
    '    drops empty figures and your prose will reference dangling figures.',
  ].join('\n')
}
