...
4. **Adapter #1 — Nous** (the substrate's first contact with real data; surface tracking gaps in `gaps.md`). Highest leverage next move — validates the schema against `~/Documents/Projects/inference-sim/`.
5. **Refresh affordances** (workspace + per-intent buttons in the chrome; staleness chip on projections). Lands alongside or after Adapter #1 since both are needed to make the read pipeline interactive.
6. **Filter / group / sort on Map** (UI-only, can run in parallel with adapter work).
7. **Adapter #2 — Paper.**
8. **Adapter #3 — Coral.**
9. **Adapter #4 — Feature.**
10. **Visual baseline regen** after each chrome change.
