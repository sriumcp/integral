...
> **v0.2.0 status note (2026-05-29).** Most of this document was written during v0.1 / v0.1.5. Three v0.2.0 changes affect what's true today: (1) the schema bumped from `0.1.0` to `0.2.0`, dropping `paper-campaign` / `paper-section` / `paper-claim` / `feature-pr` (and their fixture data); (2) the runtime fixture / `?sources=fixture` URL is gone — every source is now a real adapter; (3) visual-regression baselines were removed alongside the fixture. Items below describing fixture-dependent state, paper-* / PR-* chrome, or visual baselines describe history, not current behavior. The Nous round-trip + Coral read-only + GitHub-issues read-only adapters all survive intact. See `intent-schema-v0.2.md` for the schema diff.
...
Every chrome-affecting item regenerates the 15 baselines via `npm run test:e2e:visual:update`. Already regenerated through this expansion: A3 (refresh button in right cluster), A4 (WritebackForm appears on Nous draft Shaping), A4.6 (`+ new nous campaign` button on Map). Future regen candidates as features land:
