...
Integral is at **v0.1 expansion in flight (Path 2)**. Chrome stack landed; typed `Operation` log shipped; multi-source data plane shipped; Adapter #1 (Nous) Phases 1+2+3+4 shipped; **A4 writeback shipped** — Shaping commit on a Nous draft now writes a real `campaign-<run_id>.yaml` to a configured Nous source path. The substrate now closes the loop on Nous: declare → execute (`nous run campaign-X.yaml`, run by the user) → interpret. Track A's remaining piece is A5 (one user-fired Operation that triggers harness execution from inside Integral).
...
- **Tests**: 536 Vitest tests + 17 Playwright behavioral E2E + 15 Playwright visual baselines; all passing; TS strict clean; build clean.
