...
- **Adapter #1 (Nous, Phase 1)**: `integral-ui/src/adapters/nous/` (browser-safe types + pure interpreter) + `integral-ui/vite-plugin-nous-adapter/` (Node-only `FilesystemNousSource` + Vite middleware). The adapter follows a transport/interpreter split: `NousSource` is the abstract transport (currently `FilesystemNousSource`; v0.2 may add `S3NousSource`/`HTTPNousSource` without changing the interpreter). The pure `buildNousWorkspace` reads `campaign-X.yaml` declarations + `.nous/<run>/state.json` runtime state, produces typed `nous-campaign` Intents + IntentStates with status mapped from Nous phases. Exposed via `/api/workspace?source=nous`. UI: `?source=nous` query param flips `App.tsx` to fetch from the API and gate rendering on `WorkspaceSchema.safeParse`. Smoke test against real `~/Documents/Projects/inference-sim/` produces 20 typed campaigns. Phase 2 (iterations from `ledger.json`) and Phase 3 (principles → `KnowledgeRef`s) remain.
- **Tests**: 334 Vitest tests + 15 Playwright behavioral E2E + 15 Playwright visual baselines; all passing; TS strict clean; build clean.
