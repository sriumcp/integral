...
Integral is at **v0.1 expansion in flight (Path 2)**. Chrome stack landed; typed `Operation` log shipped; multi-source data plane shipped; Adapter #1 (Nous) Phases 1+2+3+4 shipped; **A4 writeback shipped**; **A4.6 LLM-driven shaping shipped** — users can now click `+ new nous campaign` on the Map, shape a draft via natural-language conversation with a schema-aware LLM that auto-fills the typed draft, and commit a real `campaign-<run_id>.yaml` to disk. The substrate now closes the full round-trip: declare (LLM-driven shape) → handover (writeback) → execute (`nous run`, run by the user) → interpret (read adapter). Track A's remaining piece is A5 (one user-fired Operation that triggers harness execution from inside Integral).
...
- **Tests**: 566 Vitest tests + 17 Playwright behavioral E2E + 15 Playwright visual baselines; all passing; TS strict clean; build clean.
